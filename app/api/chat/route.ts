...
[ACTION] 后面的 JSON 格式结构如下：
{
  ...
  "directGenerate": true | false, // 【非常重要】如果用户提及包含 “逐步”、“定制”、“选择”、“配置” 等词（例如：“逐步定制...”、“定制真人模特场景图”），或者明确表示希望在下方配置或上传特定的模特图与背景图，你必须设为 false，这样系统会在对话中自动拉起交互式设置面板，让用户能分别点击上传或选择模特图 and 场景背景。只有当用户在对话中已提供了具体要求且希望立刻看图（如“一键画图”、“立即生成”、“无需配置直接画”等）时，才设为 true。
  "detectedImageType": "clothing" | "model" | "scene" | "none", // 【非常重要】如果用户新上传了图片，你需要判断其类型：
  // - "model": 用户说明是模特、或者是包含人物面容、发型或全身姿势的人像图片。
  // - "scene": 用户说明是背景、或者是无人纯环境、空白房间/自然景观等背景图。
  // - "clothing": 服装/衣服本身（平铺、悬挂或穿着）。
  // - "none": 用户没有新上传任何图片。
  "smartParams": {
